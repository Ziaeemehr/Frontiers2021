...
/*------------------------------------------------------------*/
dim1 SDDE::KuramotoK1(const double t, Mdim2I &plag, int SIM_n)
{
    double sum = 0;
    dim1 k1(N);
    ...
    {
        sum = 0.0;
        for (SRD::InnerIterator it(sCij, i); it; ++it)
        {
            int k = it.col();
            double d = Dij.coeff(i, k);
            double c = it.value();
            if (d > dtmax)
            {
                sum += c * sin(interp_x(t - d, plag(i, k), k) - pt_x_ar[i][SIM_n]);
                // std::cout << i << " " << k << " " << d << "   true" << std::endl;
            }
            else
            {
...
/*------------------------------------------------------------*/
dim1 SDDE::KuramotoK2(const double t, Mdim2I &plag, int SIM_n, const double dt, const dim1 &k1)
{
    double sum = 0;
    dim1 k2(N);
...
/*------------------------------------------------------------*/
dim1 SDDE::KuramotoK3(const double t, Mdim2I &plag, int SIM_n, const double dt, const dim1 &k2)
{
    double sum = 0;
    dim1 k3(N);
...
/*------------------------------------------------------------*/
dim1 SDDE::KuramotoK4(const double t, Mdim2I &plag, int SIM_n, const double dt, const dim1 &x)
{
    double sum = 0;
    dim1 k4(N);
...
/*------------------------------------------------------------*/
void SDDE::integrate(const int numEnsembles)
{
    using namespace std;
...
/*------------------------------------------------------------*/
int SDDE::interpolate_coordinates(const double ti, const double tf,
                                  const double dtInterp, const std::string kind)
{
    /*kind could be linear or quadratic */
    // std::cout<<"interpolating ..."<<"\n";

    int n_new = (int)((tf - ti) / dtInterp);
    len_t_intp = n_new;
    int n = pt_t_ar.size(); // points we have unequal distance
    pt_t_intp = linspace(ti, tf, n_new);

    // pt_t_intp[N x n_new]
    pt_x_intp.resize(N);
    for (int i = 0; i < N; i++)
        pt_x_intp[i].resize(n_new);

    dim1 Y(n);
    //X : pt_t_ar[n]      Xnew : pt_t_intp[n_new]
    //Y : pt_x_ar[N,n]    Ynew : pt_x_intp[N,n_new]
    int n0 = 0;
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < n; j++)
        ...
        for (int k = 0; k < n_new; k++)
        {
            pt_x_intp[i][k] = sin(interpolate(pt_t_ar, Y, pt_t_intp[k], n0, kind));
        }
    }

    COORDINATES_INTERPOLATED = true;
