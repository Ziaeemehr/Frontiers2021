                   ...
                   int FRACTION)
{
    ...
    fftw_execute(plan);
    for (int i = 0; i < (NUM_POINTS/FRACTION); i++)
    {
...
void print2DVector(const dim2& vec2d, const std::string fileName)
{
    long unsigned int row = vec2d.size();
    long unsigned int col = vec2d[0].size();

    std::ofstream oX((fileName + ".bin").c_str(),
                     std::ios::out | std::ios::binary);
    if (!oX)
    {
        cout << "Could not open file for binary output!";
    ...
    for (long unsigned i = 0; i < row; i++)
        for (long unsigned j = 0; j < col; j++)
            oX.write((char *)&vec2d[i][j], sizeof(double));
