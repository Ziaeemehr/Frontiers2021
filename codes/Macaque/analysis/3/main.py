...
def calculate_distance(cor_path, nu, metric="euclidean",):
...
if __name__ == "__main__":
    ...
    max_distance = 55
    cor_path = data_path
    calculate_distance(cor_path, NU)
    ...
    ax1[0].plot(f["nu"], f["d"], marker="o", color="b", label="v=1.5")
    ...
    for ii in range(2):
        ax[ii][0].text(-0.3, 0.8,
                       labels[ii],
                       fontsize=18,
                       transform=ax[ii][0].transAxes)

    ax1[0].text(-0.1, 0.7, labels[2], fontsize=18,
                transform=ax1[0].transAxes)
    ax1[1].text(-0.15, 0.7, labels[3], fontsize=18,
    ...
    try:
        ...
        calculate_distance(cor_path, NU)
        f = np.load(join(data_path, "npz", "distance-euclidean.npz"))
        ax1[0].plot(f["nu"], f["d"], marker="*", color="r", label="v=4")

        cor_path="../../../v5/data"
        calculate_distance(cor_path, NU)
        f = np.load(join(data_path, "npz", "distance-euclidean.npz"))
        ax1[0].plot(f["nu"], f["d"], marker="s", color="gray", label="v=5")
    except:
        pass
