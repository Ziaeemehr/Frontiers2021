...
def plot_cor(cor, ax,
    ...
    im = ax.imshow(cor, origin="lower", cmap=cmap, vmax=1, vmin=-1, aspect="auto")
...
if __name__ == "__main__":
    ...
    # adj = np.loadtxt(join(data_path, connectmat_name))
    # adj = adj/np.max(adj)
    # communities = walktrap(adj, steps=4)
    ...
    for i in NU:
        ...
        cor = np.load("{:s}/npz/{:s}.npz".format(data_path,
                                                 subnamefr))['cor']
        # cor_r = reorder_nodes(cor, communities)
    ...
    calculate_cor_WD(g, network_subname, d_step=8, w_step=5)
